...
- **A facts layer.** `ats kg` keeps durable subject–predicate–object knowledge beside the tasks: agents **propose** (one triple, or a JSONL batch with `--file`), a human **ratifies** (the only write path), and `ats kg ask` answers with deterministic lexical scoring, a confidence verdict, and full provenance — no LLM, no graph server, an append-only file that travels with `ats state export`. A gate answers before anything is staged: a duplicate is a no-op, a contradiction needs `--supersedes` or `--additive`, a fact the reviewer declined needs `--acknowledge-rejected` (exit 4); `ats kg pending` shows the reviewer what each proposal would do. Retraction and supersession close a fact's validity interval instead of deleting it, `--as-of` answers what the store believed then, `ats kg history` shows a fact's chain, `ats kg nodes` shows what the store knows about whom, `ats context` carries the facts about a task, `--semantic` adds the adapter's embedder as an opt-in dense branch, and `ats kg export` speaks LadybugDB/Kùzu, Neo4j, FalkorDB and Graphiti.
...
- **The facts layer is lexical by default and human-gated.** `ats kg ask` is deterministic keyword scoring with provenance and a confidence verdict; `--semantic` adds the active adapter's embedder as an opt-in dense branch and needs one. Nothing reaches the fact store without human ratification — a burst of agent proposals waits for review by design — and nothing that duplicates, contradicts, or repeats a declined fact reaches the review queue without saying so.
...
ats kg nodes acme                                  # what the store knows about whom
ats kg history <fact-id>                           # events and supersession chain
ats kg export --cypher > facts.cypher      # load into LadybugDB / Kùzu, full provenance on every fact
ats kg export --dialect falkordb           # openCypher for FalkorDB / Neo4j, re-runnable
