# Refactor Plan — ~/ticktick-cli → @reneza/akb-*
...
| `lib/corpus-cache.js`      | `core/corpus-cache.js`                              | Path constant changes to `~/.config/akb/corpus-cache.json` (env override `AKB_CORPUS_CACHE`). |
| `lib/usage-log.js`         | `core/usage-log.js`                                 | Path constant changes to `~/.config/akb/search-log.jsonl`. |
...
| `bin/ticktick.js`          | `cli/bin/akb.js`                                    | Subcommands delegate to `core` for retrieval, to the active adapter for storage. |
| `bin/ticktick-mcp.js`      | dropped (or `cli/bin/akb-mcp.js`, deferred)        | MCP server stays out of v0.1. |
...
4. **`packages/cli/`** — `bin/akb.js` parser + dispatcher. Loads the active adapter via `~/.config/akb/config.json`. Delegates retrieval to core, storage to adapter.
5. **`packages/core/bench/`** — copy `bench/` from `~/ticktick-cli/bench/`. Adapter-agnostic.
6. **End-to-end smoke test** — install `npm install` at the root (workspaces), then `node packages/cli/bin/akb.js find "ffmpeg commands"`.
...
- Vector index management commands (`tasks vector-sync`) — wraps `adapter-ticktick/embedding.js` but the CLI subcommand surface stays adapter-namespaced (`akb adapter-cmd <args>`).
