...
- **A gate in front of every proposal.** `ats kg propose` checks the triple against the store and the review queue before staging: a duplicate of an active or already-queued fact is a no-op (`staged: false`, exit 0); a contradiction — same subject and predicate, another object — is refused unless `--supersedes <id>` names the fact it replaces or `--additive` declares a multi-valued predicate; a triple the reviewer already declined is refused until `--acknowledge-rejected <id>` names that decision. A refusal prints the report and exits 4, an exit code an agent can branch on next to 3 for a failed `--if-match`. Matching is normalized (case, whitespace, trailing punctuation) and scoped to the domain.
- **Supersession.** A proposal with `--supersedes` closes the old fact and adds its replacement in one ratification, written as one append: the old fact reads `superseded` with `supersededBy`, the new one carries `supersedes`. A closed fact is never closed twice — retracting or superseding a fact that is already retracted or superseded is refused at proposal time and again at ratification, so two retractions staged before either lands cannot move the first closing time. `ats kg stats` counts `superseded` beside `retracted`.
- **Point-in-time asks and history.** `ats kg ask … --as-of DATE` and `ats kg facts --as-of DATE` answer from the validity intervals — what the store believed at that instant, closed facts included; a bare date means the end of that day. `ats kg history FACT_ID` lists the events that touched a fact and its supersession chain in both directions.
- **Facts inside `ats context`.** The context bundle carries `facts`: those proposed from the task (`ats kg propose … --task P/T`) as `linked`, then the best lexical matches on the task's title and intent as `related`, each with provenance. `--no-facts` leaves the layer out; `--facts-limit` and `--domain` scope it.
- **Batch proposals.** `ats kg propose --file FILE` (or `--file -` for stdin) reads one JSON object per line — `{subject, predicate, object, domain?, source?, confidence?, task?, supersedes?, additive?, acknowledgeRejected?}` — and stages every line on its own through the gate: bad lines are named, in-batch duplicates are no-ops, and `--domain` / `--source` / `--confidence` fill what a line does not carry. The report exits 0 when every line staged or was already known, 4 when any line was refused or unreadable.
- **Export dialects and Graphiti episodes.** `ats kg export --dialect ladybug|kuzu|neo4j|falkordb`: the embedded engines keep the typed DDL; Neo4j and FalkorDB get openCypher with an index on `Entity.name`, entities MERGEd by name and every fact MERGEd by id with its properties SET, so the same script runs again after facts close or arrive. `ats kg export --graphiti` emits episode JSONL — `name`, `content` (the fact as a sentence; a closed fact says so), `source`, `source_description` (the provenance record), `reference_time`, `group_id`, `uuid`, validity — ready for `add_episode_bulk`. Every export now carries `supersedes` / `supersededBy`.
- **The reviewer's view: `ats kg pending`.** Every fact proposal not yet in the store (pending, or approved and not ratified), grouped by domain, with what ratifying it would do (`add`, `add, superseding <id>`, `add (additional value)`, `retract <id>`) and a verdict against the store as it is now: `clear`, `duplicate` (its twin got ratified meanwhile), `contradiction`, `rejected`, or `stale` (the fact it retracts or supersedes is already closed).
- **A confidence verdict on every ask, and `--semantic`.** `ats kg ask` returns `confidence: { verdict, reason, coverage }` — `strong`, `moderate`, `weak`, `none` — read from how much of the question the top fact carries. `--semantic` (or `ATS_KG_ASK_SEMANTIC=1`; `--lexical` overrides) adds a dense branch over the same facts through the active adapter's embedder — the contract's `embeddings(texts)`, or the TickTick adapter's Ollama client behind `__ext.embedding` — fused with the lexical branch by reciprocal rank fusion, with `confidence` read from branch agreement the way `find` does. Fact vectors are cached per embedder (`kg-vectors.json`, `ATS_KG_VECTORS`), so a repeat ask embeds only the question; a failing embedder degrades the ask to the lexical branch and says so (`degraded`, `branches`). Lexical stays the default.
- **An entity view.** `ats kg nodes [QUERY]` lists the subjects and objects the store knows with how much it knows about each — fact count, which side, domains, predicates — most-known first; spellings that normalize alike are one entity. `ats kg facts --entity NAME` reads both directions, and `ats kg ask … --center NAME` anchors an answer on one entity.
- **Work streams: `ats workstream`.** One parent task plus two or three sub-tasks, each carrying a review date and a named verification, behind a gate that opens only once every item has a recorded PASS with evidence — what reaches a human is small, scannable, dated, and already verified. It stores nothing of its own: membership is real sub-tasks, outcome and done-when are `intent` metadata, the verification log is the action ledger, the review date is the task due date, so `ats intent get`, `ats hierarchy get`, `ats ledger list` and `ats undo` read a work stream as an ordinary task graph. `ats workstream template | lint | create | verify | ready` (`lint` exits 2 until the spec is clean; `ready` exits 0 once every item passed). Adapter capabilities behind it, all additive: `tasks create --parent <id>` creates a real nested sub-task; `tasks get|update --live` bypasses the local cache and drops the cached merge base on update; `tasks create|update --raw` is the per-call form of `format-skip`; the TickTick mapping surfaces `parentId` and `childIds`; vector sync honors an injected embedder; highlight colour follows gate state (cyan outcome, yellow review date, green verified, red failed).
